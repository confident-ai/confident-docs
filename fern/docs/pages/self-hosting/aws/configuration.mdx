...
| ----------- | -------------- | ---- | ------ | ------------- |
| Staging     | `m6i.2xlarge`  | 8    | 32 GB  | 2-8           |
| Production  | `m6i.2xlarge`  | 8    | 32 GB  | 2-8           |
| High volume | `m6i.4xlarge`  | 16   | 64 GB  | 4-12          |
...
| `confident_psql_username`             | Master username for the database         |
| `confident_psql_password`             | Master password for the database         |
...
| Variable                      | What it controls                                                  |
| ----------------------------- | ----------------------------------------------------------------- |
| `confident_application_code`  | Short code used in resource identifiers (e.g., RDS instance name) |
...
| `confident_tags`              | Tags applied to all AWS resources                                 |

<Note>
  **When to change these:** You typically don't need to modify these unless your organization
  has specific resource naming or tagging requirements.
</Note>

## EKS access configuration

```hcl
confident_public_eks   = false
confident_eks_admin_arn = ""
```

| Variable                 | What it controls                                                       |
| ------------------------ | ---------------------------------------------------------------------- |
| `confident_public_eks`   | When `true`: EKS API is publicly accessible, ALB is internet-facing (HTTP only), ACM/DNS disabled. When `false` (default): private setup with ACM certificates and DNS management. |
| `confident_eks_admin_arn` | IAM role ARN to grant EKS cluster admin access (e.g., a shared platform team role) |
...
```hcl
confident_rds_maintenance_window     = "Mon:00:00-Mon:03:00"
confident_rds_backup_window          = "03:00-06:00"
confident_rds_backup_retention_period = 7
```

| Variable                              | What it controls                                     |
| ------------------------------------- | ---------------------------------------------------- |
| `confident_rds_maintenance_window`    | Weekly window for automated RDS maintenance (UTC)    |
| `confident_rds_backup_window`         | Daily window for automated backups (UTC)             |
| `confident_rds_backup_retention_period` | Number of days to retain automated backups (1-35)  |

<Note>
  **Schedule maintenance during low-traffic periods.** The defaults use early Monday morning UTC.
  Adjust to match your team's off-peak hours.
</Note>
...
| Variable                     | What it controls                                        |
| ---------------------------- | ------------------------------------------------------- |
| `confident_test_cases_bucket` | Suffix for the test cases S3 bucket name               |
| `confident_payloads_bucket`   | Suffix for the payloads S3 bucket name                 |
...
confident_clickhouse_storage_class    = "clickhouse-gp3"
```
...
| `confident_clickhouse_replicas`         | Replicas per shard (2 recommended for 2-AZ clusters)     |
| `confident_clickhouse_storage_size`     | Persistent volume size per ClickHouse pod                 |
| `confident_clickhouse_storage_class`    | StorageClass for ClickHouse PVCs (gp3 encrypted)         |
...
```hcl
confident_clickhouse_backup_bucket_name = "ch_backups"
confident_clickhouse_backup_schedule    = "0 2 * * *"
```

| Variable                                 | What it controls                              |
| ---------------------------------------- | --------------------------------------------- |
| `confident_clickhouse_backup_bucket_name` | Suffix for the ClickHouse backup S3 bucket   |
