---
slug: self-hosting/aws/kubernetes-deployment
...
<Step title="Add the ACM certificate ARN">
...
<Warning>
  **The certificate ARN must be exact.** Copy it carefully from `terraform
  output -raw acm_certificate_arn`. Don't include any trailing `%` characters
  your terminal might display.
</Warning>

</Step>

<Step title="Set ALB scheme">
...
<Note>
  **Internal vs. internet-facing:** - **Internal:** ALB only gets a private IP.
  Users must be on VPN or connected network to access. More secure for
  enterprise deployments. - **Internet-facing:** ALB gets a public IP. Anyone
  with the URL can reach the login page (authentication still required). Simpler
  but larger attack surface.
</Note>

</Step>

<Step title="Update ExternalDNS annotation (optional)">
...
<Warning>
  **Secret name must exactly match.** If the name doesn't match what Terraform
  ...
  the secret name in AWS Secrets Manager console or via: ```bash aws
...
After all services are running and healthy, proceed to [Verification](/self-hosting/aws/verification) to test the deployment end-to-end.
