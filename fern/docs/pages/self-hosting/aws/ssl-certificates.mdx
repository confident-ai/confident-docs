---
slug: self-hosting/aws/ssl-certificates
...
<Accordion title="Cloudflare">
  ...
  Target: paste the validation value 6. **Proxy status: DNS only** (gray cloud)
  — this is critical 7. TTL: Auto 8. Save

{" "}

<Warning>
  The proxy must be OFF for ACM validation. If the cloud icon is orange, click
  it to turn it gray.
</Warning>
...
<Accordion title="GoDaddy">
  ...
  suffix if present) 5. Value: paste the validation value 6. TTL: 600 seconds 7.
  Save GoDaddy often strips the trailing dot automatically.
</Accordion>
...
After the certificate is validated and issued, proceed to [Cluster Access](/self-hosting/aws/bastion-access) to configure kubectl access to your EKS cluster.
