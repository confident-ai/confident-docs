---
slug: self-hosting/aws/provisioning
...
- S3 buckets (test cases, payloads, ClickHouse backups) with VPC endpoint for private access
- Secrets Manager with KMS encryption
- IAM roles with EKS Pod Identity for pod-level AWS permissions
...
| **Storage**    | S3 buckets (test cases, payloads, ClickHouse backups), bucket policies        |
...
- `iam:CreateOpenIDConnectProvider` (for EKS Pod Identity)

<Warning>
  **Many organizations restrict IAM creation.** If you can't get these
  permissions, you may need a platform team member to run the deployment or
  pre-create the required roles.
</Warning>
...
<Warning>
  **Quota increases can take hours to days.** If you're in a new AWS account,
...
- **S3 Buckets** — private buckets for test cases, payloads, and ClickHouse backups
...
- **IAM Roles** — separate roles for EKS, nodes, and pods (EKS Pod Identity)
...
After infrastructure is provisioned, proceed to [SSL Certificates](/self-hosting/aws/ssl-certificates) to validate your HTTPS certificate.
