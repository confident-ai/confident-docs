---
slug: self-hosting/gcp/tls-certificates
description: Configure TLS certificates for Confident AI on GCP
---
...
<Note>
  **If using public GKE mode:** If you configured `confident_public_gke = true`
...
<Note>
  **GCP Network Load Balancers use IP addresses** (not hostnames like AWS ALB). You'll
  create A records (not CNAME records) pointing your domains to this IP.
</Note>

## Next steps

After cert-manager is operational and a ClusterIssuer is configured, proceed to [Cluster Access](/self-hosting/gcp/cluster-access) to configure kubectl access and verify infrastructure components.
