...
For production, enable the nightly ClickHouse backup to Blob storage, authenticated with the storage connection string. Provision the backup container and see the [Disaster Recovery](/self-hosting/disaster-recovery) page for the credential detail. Add this under your existing `clickhouse:` block and `helm upgrade`:
