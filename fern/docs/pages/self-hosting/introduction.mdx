---
slug: self-hosting
description: Run Confident AI entirely inside your own cloud account
---
...
<Tip>
Self-hosting is available on Enterprise plans. [Talk to the platform team](https://www.confident-ai.com/book-a-demo) to get a license key and access to the container images.
</Tip>
...
<Step title="Deploy the application with Helm">
...
<CardGroup cols={3}>
  <Card
    title="AWS"
    icon={
      <svg viewBox="0 -30 150 150" xmlns="http://www.w3.org/2000/svg">
        <path
          fill="#F90"
    ...
    href="/self-hosting/aws/overview"
  >
    EKS, RDS PostgreSQL, and S3, with EKS Pod Identity for keyless access.
  </Card>
  <Card
    title="GCP"
    icon={<img src="../../assets/icons/gcp.svg" alt="GCP logo" />}
    href="/self-hosting/gcp/overview"
  >
    GKE, Cloud SQL, and GCS, with Workload Identity for keyless access.
  </Card>
  <Card
    title="Azure"
    icon={<img src="../../assets/icons/azure.svg" alt="Azure logo" />}
...
<Note>
- **Image pull credentials** (`imagePullSecrets`): the first-party images are hosted in Confident AI's private registry. You get credentials to pull them into your cluster.
- **License key** (`CONFIDENT_LICENSE_KEY`): a signed key that enables the features your plan includes. Without it, the app starts with all features off.
</Note>
