...
{   
    ...
    "prompts": [{"alias": "...", "version": "..."}] // Optional, might not be sent if prompts aren't configured
}
```
</CodeBlock>

The `input` and `context` will correspond to the values in your goldens. Since `context` is an optional field, your LLM connection should be flexible in parsing it. You can also [configure prompts](#configure-prompts) in the LLM connection settings, which will be included in the request data as `prompts`, a list of objects with the prompt `alias` and `version`.
...
2. The **JSON key path** to the mandatory `actual_output` parameter, and the optional `retrieval_context` , and `tools_called` parameters. The JSON key path is a list of strings.
...
The alias and version of the configured prompts [will be sent](#parse-request-data) to your LLM endpoint as part of the request data.

<Tabs>
  <Tab title="Configure Prompts">
    <Frame caption="Configure Prompts">
      <video
        src="https://confident-docs.s3.us-east-1.amazonaws.com/llm-connection%3Aconfigure-prompts.mp4"
        autoPlay
      />
    </Frame>
  </Tab>
  <Tab title="Prompts in Test Runs">
    <Frame caption="Prompts in Test Runs">
      <video
        src="https://confident-docs.s3.us-east-1.amazonaws.com/llm-connection%3Atest-run-prompts.mp4"
        autoPlay
      />
    </Frame>
  </Tab>
