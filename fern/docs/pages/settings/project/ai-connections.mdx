...
You can choose between _HTTP Response_, _HTTP Streaming_, and _SSE Streaming_ modes.

- **HTTP Response**: The endpoint returns a HTTP response containing the actual output.
- **HTTP Streaming**: The endpoint returns a stream of HTTP responses containing the actual output.
- **SSE Streaming**: The endpoint returns a stream of SSE events containing the actual output.
...
<Tip>
If your endpoint returns a custom response format like JSON or any other formats, you can configure the actual output key path or use a transformer to extract the actual output from the response. See [Actual Output Key Path](#actual-output-key-path) and [Transformers](#transformers) for more details.
</Tip>

### Streaming Mode

For _SSE Streaming_ and _HTTP Streaming_ modes, your endpoints can return either strings, JSON objects, or custom formats that can be processed using transformers for each chunks.
...
<Tabs>
  ...
  <Tab title="JSON">
    For JSON objects, Confident AI parses the JSON object for each chunk and returns the final output.
    ...
    ```
    Confident AI response after extraction: 
...
| `threadId`                                 | Unique identifier for grouping traces from multi-turn red-team attacks into a single thread | string     |
...
| `threadId`        | Unique identifier for grouping traces from multi-turn red-team attacks into a single thread | `Optional[str]`            |
...
## Linking Risk Assessments to Threads

For multi-turn red-team attack methods (e.g. _Linear Jailbreaking_, _Crescendo Jailbreaking_, _Tree Jailbreaking_, _Sequential Jailbreak_, _Bad Likert Judge_), Confident AI calls your endpoint once per turn — every call produces its own trace. To group all of those turn-traces into a single conversation in the assessment view, the platform sends a `threadId` in the payload. Stamp it onto your traces and the assessment side drawer will open them as one thread.
...
sequenceDiagram
    ...
    Note over C,E: Turn 1
    C->>E: { threadId, ...payload }
    E->>T: Create trace with threadId
    E-->>C: Return actual_output
    ...
    T-->>C: Send traces to Confident AI
    Note over C: All turn-traces grouped under one thread
```
...
@observe()
def llm_app(input: str, thread_id: str):
...
def generate(request: dict):
    ...
    response = llm_app(request["input"], thread_id)
    return {"output": response}
```
...
<Tip>
  Once linked, the assessment side drawer for a multi-turn test case shows
  "View thread" instead of "View trace" — opening it surfaces the full
  multi-turn conversation in a single drawer.
</Tip>
