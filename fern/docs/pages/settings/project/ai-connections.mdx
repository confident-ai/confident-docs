...
### Linking Individual Turns to Traces (Multi-Turn)

For multi-turn evaluations, Confident AI calls your endpoint once per turn. Each turn has its own `turnId` that you can pass to your tracing setup alongside `testCaseId`. This links each turn's trace to the specific turn in the conversation, letting you view traces per-turn from the evaluation results.
...
sequenceDiagram
    ...
    C->>E: { testCaseId, turnId, ...payload }
    E->>T: Create trace with testCaseId + turnId
    E-->>C: Return actual_output
...
Include both `testCaseId` and `turnId` in your payload configuration:
...
Then, pass both identifiers to your tracing implementation:
...
@observe()
def llm_app(input: str, test_case_id: str, turn_id: str):
    update_current_trace(test_case_id=test_case_id, turn_id=turn_id)
...
def generate(request: dict):
    ...
    response = llm_app(request["input"], test_case_id, turn_id)
    return {"output": response}
```
...
def generate(request: dict):
    ...
    result = agent_executor.invoke(
        {"input": request["input"]},
        config={"callbacks": [CallbackHandler(test_case_id=test_case_id, turn_id=turn_id)]}
    )
    return {"output": result["output"]}
```
