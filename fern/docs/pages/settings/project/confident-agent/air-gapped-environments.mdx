---
subtitle: Deploy the Confident Relay Agent in air-gapped or egress-restricted networks.
slug: settings/project/confident-agent/air-gapped-environments
...
<Step title="Restricted: whitelist Confident AI's hostname">
...
<Warning>
  Confident AI's IPs can change. IP-based allowlisting is brittle and may break connectivity if the underlying infrastructure rotates. Prefer hostname-based rules, or allow all outbound on `443`, whenever your policy allows it.
</Warning>
...
<Tip>
  Need the IP address for an IP-based egress allowlist? **Reach out to the Confident AI team** and we will provide the current IP(s) for the relay endpoint.
</Tip>
