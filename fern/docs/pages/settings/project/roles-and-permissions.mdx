...
- `merge` — Merge changes into the target resource (used for `promptBranch`)
...
- `evaluationRule` — Rules that automatically run metric collections on incoming traces, spans, and threads
...
- `promptBranch` — Prompt branches and pull requests; `create` (create branches and PRs), `update` (rename branches, commit to branches, edit PR descriptions), `merge` (merge PRs into main), and `delete` (delete branches, close PRs).
