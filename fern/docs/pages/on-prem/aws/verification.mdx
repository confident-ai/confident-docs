...
<Note>
**S3 is used for file uploads.** If S3 connectivity fails, users won't be able to upload datasets or export reports. The backend uses IRSA (IAM Roles for Service Accounts) to access S3—verify the `confident-s3-sa` service account exists.
</Note>
