...
The command exits with code `0` only when the policy is fully met, and a non-zero code otherwise — so a failing policy stops your pipeline.

Under the hood this calls the [`POST /v1/governance/assess`](/api-reference) endpoint with your project's API key. The response tells you whether the policy passed and which policy was evaluated:
