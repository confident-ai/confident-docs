...
const generate = async (prompt: string): Promise<string> => {
  updateCurrentSpan({
    testCase: new LLMTestCase({ input: prompt, actualOutput: llmResponse }),
  });
  return "LLM response";
};
...
Running evals on traces are akin to running [end-to-end evals](/docs/llm-evaluation/single-turn/end-to-end), where you disregard the performance of individual spans within the trace and treat your application as a black-box.
