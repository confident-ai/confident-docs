...
Confident AI exposes two project-wide sample rates that control what fraction of your ingested data is automatically evaluated:
...
<Note>
  These project-wide rates decide only *which* traces or threads are evaluated — not which metrics run. Once an item is sampled in, each metric in the collection still follows its own **Sample Rate**, so the two compound. To score every metric on a sampled item, keep the per-metric sample rates at `1.0`.
</Note>
