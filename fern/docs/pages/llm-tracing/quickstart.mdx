...
3. The outermost observed function becomes the **trace** — all nested spans roll up into it (see [troubleshooting](/docs/llm-tracing/troubleshooting) if spans are creating separate traces instead of nesting)
...
Install the [`deepeval-tracing` Agent Skill](https://github.com/confident-ai/deepeval/tree/main/skills/deepeval-tracing) with any [Skills](https://github.com/anthropics/skills)-compatible installer. This works with Cursor, Claude Code, Codex, Windsurf, OpenCode, and any other assistant that supports the Skills standard:

```bash
npx skills add confident-ai/deepeval --skill deepeval-tracing
```

The skill teaches your agent how to choose between a native integration and manual `@observe`, set span types/tags/metadata, and send traces to Confident AI's Observatory. It triggers automatically on prompts like the ones below.
...
See the [troubleshooting page](/docs/llm-tracing/troubleshooting#traces-not-showing-up) for more details.
...
Both can be called multiple times from anywhere inside an observed function — values are merged, with later calls overriding earlier ones. Make sure to use the right one — see [`update_current_trace` vs `update_current_span`](/docs/llm-tracing/troubleshooting#update_current_trace-vs-update_current_span) in the troubleshooting page.
