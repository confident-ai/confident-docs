...
You can attach custom metadata to a thread to tag production conversations with attributes like DVA version, client, or agent ID. Thread metadata is filterable and groupable across the observatory, which makes it easy to slice production traffic by version or customer.

Thread metadata is set by including a `thread.metadata` object on any trace you ingest into the thread. The values can be any JSON-serializable type — they're stringified server-side for storage.

```json title="POST /v1/traces"
{
  "uuid": "<TRACE-UUID>",
...
<Note>
  `thread.metadata` only takes effect when `threadId` is also set on the trace.
  Requests with `thread.metadata` but no `threadId` are rejected with a 400.
</Note>

Successive ingestions for the same `threadId` **merge** their metadata keys, so you can build up a thread's metadata incrementally across turns. Sending the same key again overwrites the previous value:

```json title="Subsequent trace — merges into existing thread metadata"
{
...
<Tip>
  You can also view and edit a thread's metadata directly from the platform via
  the **Metadata** tab on the thread details page.
</Tip>
