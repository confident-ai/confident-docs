...
<Tip>
  To evaluate threads automatically without calling the evaluate thread function
  from your code, configure [Evaluation
  Rules](/docs/settings/project/evaluation-rules) in Project Settings—a thread
  rule waits for the conversation to be idle for a configurable time limit, then
  runs your multi-turn metric collection.
</Tip>
...
import { observe, updateCurrentTrace, evaluateThread } from "deepeval/tracing";
