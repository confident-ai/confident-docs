...
<Callout>
  Each metric requires different test case parameters. For detailed information
  on required test case parameters for each metric, refer to the [official
...
<Callout>
  The **input** and **output** accept `Any` type for visualization purposes, but
  we recommend setting them as strings for running evaluations.
</Callout>
...
<Callout type="info">
  `update_current_trace` can be set multiple times at any point in your code
  under the `observe` decorator, which is useful when a parameter is only
  accessible in specific parts of your code.
</Callout>
