...
<Tabs>
    ...
    <Tab title="TypeScript" language="typescript">
            ```typescript title="index.ts" {6}
            import { observe, updateLlmSpan, Prompt } from "deepeval-ts/tracing";
            const confidentPrompt = Prompt({alias: "YOUR_PROMPT_ALIAS"})
            await confidentPrompt.pull()
