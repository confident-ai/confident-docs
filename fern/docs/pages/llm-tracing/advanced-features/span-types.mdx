...
<Callout>
  Anything that is **NOT** one of these four default span types are
  **automatically custom spans.**
</Callout>
...
<Callout>
  If `cost_per_input_token` is not set, setting `input_token_count` will not
  help calculate the cost. The same applies to output tokens.
</Callout>
