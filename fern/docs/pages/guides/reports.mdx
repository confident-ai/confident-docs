...
Reports are written by your project's **platform model** — the model that powers Confident AI's own AI features, like classification, summaries, and report generation. This is a **separate setting** from the [evaluation model](/docs/settings/project/evaluation-models) that scores your LLM-as-a-judge metrics, so changing one never touches the other.

Set or change it in **Project Settings** → **Platform Model**. If you haven't set a project-specific one, reports use your organization's default platform model.
...
<CardGroup cols={2}>
  ...
  >
    Configure the model and credentials behind your LLM-as-a-judge metric
    scoring, including the `gpt-5` verification note.
  </Card>
</CardGroup>
