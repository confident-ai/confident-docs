...
subtitle: Open a test run, stream your app's traces into it as test cases, and let Confident AI evaluates each one at the trace and component level.
...
- **Send test cases over OpenTelemetry** with span attributes, for users who already have an instrumentation in place.
...
- **A Project API Key** — `CONFIDENT_API_KEY` (e.g. `confident_us_proj_...`). [Grab yours here](/docs/api-reference/authentication).
...
Both fields are optional. `identifier` is a label so you can find the run again later; `metricCollection` sets a **default** that evaluates every case you don't point at a collection of its own. You get back the `id` and a `link` to the run on the platform:
...
To answer that, you can evaluate your **spans**, give each one its own `metricCollection`. Each span is a component, evaluated on its own and can be seen in the observatory or the test case's trace view.
...
If your app already emits [OpenTelemetry](/docs/integrations/opentelemetry) spans, you can easily carry the same fields as span **attributes**, export the spans to Confident AI's otel endpoint.

The moves are identical to the walkthrough above. You still [open the run over the API](#open-a-test-run) to get a `testRunId`; from there, everything travels as attributes on the spans you already emit. Point your OTLP/HTTP exporter at Confident AI:

```bash
https://otel.confident-ai.com/v1/traces
```

authenticate with the `x-confident-api-key` header, and set two groups of attributes:
...
OpenTelemetry is language agnostic, so you can always send the traces from your app natively and still get the same test runs as the API or regular SDK users. Here's a few examples in different languages:
