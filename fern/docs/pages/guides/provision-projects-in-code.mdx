...
description: Isolate tenant traces, datasets, and evaluations by provisioning one Confident AI project per application and routing traces to the right project.
---

## Overview

This guide is for teams that want to create Confident AI projects programmatically instead of creating them manually in the platform UI. Use it for multi-tenant products, proofs of concept, or internal platforms where each tenant, customer, or application should have its own isolated Confident AI project.

This guide walks through the full workflow: configure the [Admin SDK](/docs/settings/project/management/introduction) with one **Organization API Key**, create projects in code, store each returned Project API Key, and use that key to route traces into the correct project.

In this guide, you will:

- Configure the Admin SDK with an Organization API Key
- Create a Confident AI project for each tenant or application
- Store the project ID and returned Project API Key
- Route traces for each tenant or application into its own project
...
Continue with the related Admin SDK and tracing sections:
...
<CardGroup cols={2}>
  ...
  >
    Update, list, and delete projects.
  </Card>
  <Card
  ...
  <Card title="LLM Tracing" icon="route" href="/docs/llm-tracing/quickstart">
    Learn about spans, span types, and trace enrichment.
  </Card>
  <Card
    title="Online Evals"
    icon="toggle-on"
  ...
  >
    Automatically evaluate traces as they land in each project's workspace.
  </Card>
</CardGroup>
