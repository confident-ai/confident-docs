...
<Accordion title='Passing trace attributes `Agent` constructor'>
  ```python
  from deepeval.integration.pydantic_ai import Agent
  ...
  <Note>
    All the attributes (such as `name`, `tags`, `metadata`, `thread_id`, `user_id`) that passed in the `run_sync`, `run` or `run_stream` method can will override the subsequent attributes passed in the `Agent` constructor with `trace_` prefix.
  </Note>
</Accordion>
...
Since Confident AI has an Opentelemetry compatible endpoint, you can use Pydantic AI's built-in ability to log traces in more detail by instrumenting Pydantic AI.

<Warning>
  Attributes and Evals for Pydantic AI are **NOT supported** when using verbose tracing.
</Warning>
