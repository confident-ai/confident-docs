...
## Verbose logging 
Since Confident AI has an Opentelemetry compatible endpoint, you can use Pydantic AI's in-built ability to log verbose information. 
...
<Warning>
  Evals for Pydantic AI are NOT supported when using verbose logging.
</Warning>
...
Threads are used to group related traces together, and are useful for chat apps, agents, or any multi-turn interactions. Learn more about [threads](/docs/llm-tracing/advanced-features/threads). Pass the `thread_id` to the `run_sync` (or `run` for async mode) method.
...
Other trace attributes can also be passed to the `run_sync` (or `run` for async mode) method.
