...
<Steps>
  ...
  <Step title="Configure Pydantic AI">
    Use DeepEval's Pydantic AI `Agent` to trace the LLM operations. 
    <Tabs>
      <Tab title="Synchronous">
        ```python main.py {1}
        from deepeval.integration.pydantic_ai import Agent
...
system_prompt = prompt.interpolate(name="John")

agent = Agent(
    "openai:gpt-4o-mini",
    system_prompt=system_prompt,
    llm_prompt=prompt, # for logging the prompt in the span
)
...
<CodeBlocks>
  ...
  <CodeBlock title="Asynchronous">
    ...
    async def execute_agent():
        return await agent.run("What are the LLMs?", thread_id="test_thread_id_1")
  ...
  <CodeBlock title="Streaming">
    ...
    async def execute_agent_stream():
      async with agent.run_stream("What are the LLMs?", thread_id="test_thread_id_1") as result:
          async for chunk in result.stream_text(delta=True):
              print(chunk, end="", flush=True)
          final = await result.get_output()
          print("\n\nFinal:", final)
...
Other trace attributes can also be passed to the `run_sync`, `run` or `run_stream` method.
...
You can also optionally pass trace parameters into the `Agent` class instead via `trace_[parameter name]` prefix.

<Tip>
  All the attributes (such as `name`, `tags`, `metadata`, `thread_id`,
  `user_id`) that passed in the `run_sync`, `run` or `run_stream` method can
  will override the subsequent attributes passed in the `Agent` constructor with
  `trace_` prefix.
</Tip>
...
agent = Agent(
  ...
  trace_tags=["Tag 1", "Tag 2"],
  trace_metadata={"Key": "Value"},
  trace_thread_id="thread_1",
  trace_user_id="user_1",
)
...
You can also use Pydantic AI's built-in ability to log traces in more detail by instrumenting via OTEL with Confident AI's OTEL capabilities.

<Warning>
  Attributes and Evals for Pydantic AI are **NOT supported** when using verbose
  tracing.
</Warning>

```python main.py {1,4}
from pydantic_ai import Agent
from deepeval.integrations.pydantic_ai import otel_instrument_pydantic_ai

otel_instrument_pydantic_ai()

agent = Agent(model="openai:gpt-4o-mini", system_prompt="Be concise, reply with one sentence.")
result = agent.run_sync("What are the LLMs?")
```

You might want to use the OTEL instrumentation if you prefer to import modules such as `Agent` directly from Pydantic AI instead. However you will lose a significant amount of eval features by doing so.
  ...
  <Step title="Run evals">
    ...
    <Tabs>
      ...
      <Tab title="Tool Span">
        ...
        <Note>
          `metric_collection` and `metrics` can be passed even if you are using `@agent.tool` [decorator](https://ai.pydantic.dev/tools/).
        </Note>

        <CodeBlocks>
          <CodeBlock title="Using @observe decorator">
            ```python
          ...
          <CodeBlock title="Using @tool decorator">
            ```python
            from deepeval.integrations.pydantic_ai import Agent

            weather_agent = Agent("openai:gpt-4o-mini", deps_type=Deps, ...)
