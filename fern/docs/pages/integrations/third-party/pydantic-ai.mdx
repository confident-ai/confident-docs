...
```python main.py focus={2, 7-9, 13-14}
from pydantic_ai import Agent
...
Since Confident AI has an Opentelemetry compatible endpoint, you can use Pydantic AI's built-in ability to log traces in more detail by specifying `otel=True`.
...
```python focus={2, 7-11, 14} maxLines=100
from pydantic_ai import Agent
