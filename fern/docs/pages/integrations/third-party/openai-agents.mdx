...
<Note>
  - Grouping of all the runs will only work if either `group_id` or `metadata`, or both are set.
  - If `thread_id` or `metadata` is set for an individual run, it will override the trace's `group_id` or `metadata` that was set in the `trace` block.
</Note>

### Trace input and output
In general the trace input and output will be same as input and output of agent's run. However, if you are using `trace` context to set the trace attributes, the trace input will be the **input of the first run** and the trace output will be the **output of the last run**.

If you want to set custom input and output for the trace, you can use the `update_current_trace` function.
...
Use `update_current_trace` to overwrite the trace attributes set earlier, like `name`, `metadata`, etc.
...
async def main1():
    with trace(workflow_name="test_workflow_1", group_id="test_group_id_1", metadata={"test_metadata_1": "test_metadata_1"}):
        user_query = "What's the weather like in London today?"
        update_current_trace(name="new name", metadata={"test_metadata_2": "test_metadata_2"})
```
