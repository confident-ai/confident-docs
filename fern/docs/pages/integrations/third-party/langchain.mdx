...
For tool spans, you can set the `metric_collection` name in the `metric_collection` parameter of the tool decorator imported from `deepeval.integrations.langchain`.
...
```
<Note>
  If your application does not use `@tool` decorator, you can also use `@observe` decorator from `deepeval.tracing` to set the `metric_collection` name.
  ```python
