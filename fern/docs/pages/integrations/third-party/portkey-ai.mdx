...
If you are [managing prompts](/docs/llm-evaluation/prompt-management/version-prompts) on Confident AI and wish to log them, pass your `Prompt` to the `create` method.
...
<Note>This is an example of using `STRING` type prompt interpolation.</Note>
...
<Frame caption="Create metric collection">
  <video data-video="integrations.createMetricCollection" controls autoPlay />
</Frame>
...
<Success>
  This will automatically generate a test run with evaluated Portkey traces
  using inputs from your dataset.
</Success>
