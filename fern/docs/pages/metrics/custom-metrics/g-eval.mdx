...
If you're creating G-Eval locally via `deepeval`, you can use the `upload` method to create a metric on Confident AI, or the `pull` method to fetch an existing one back into your local metric instance.
...
You can create G-Eval metrics locally using `deepeval` and upload them to Confident AI, or pull a metric that already exists on Confident AI into a local instance.
...
If a metric already exists on Confident AI, you can pull it into a local `GEval` instance by name. The `.pull()` method populates `criteria`, `evaluation_steps`, `evaluation_params`, and `rubric` from the platform, so you only need to provide the metric `name`:
...
If a multi-turn metric already exists on Confident AI, pull it into a local `ConversationalGEval` instance by name. The `.pull()` method populates `criteria`, `evaluation_steps`, `evaluation_params`, and `rubric` from the platform:
...
Under the hood, `.pull()` retrieves an existing G-Eval metric by name from your Confident AI project.

<Tabs>
  <Tab title="Single-turn">
    <EndpointRequestSnippet endpoint="GET /v1/metric/{name}" example="Single-Turn" />
  </Tab>
  <Tab title="Multi-turn">
    <EndpointRequestSnippet endpoint="GET /v1/metric/{name}" example="Multi-Turn" />
  </Tab>
</Tabs>
