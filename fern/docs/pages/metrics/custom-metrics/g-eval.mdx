...
If a metric already exists on Confident AI, you can pull it into a local `GEval` instance by name. The `.pull()` method populates `criteria`, `evaluation_steps`, `evaluation_params`, and `rubric` from the platform:
