...
The contextual recall metric is a single-turn RAG-based metric that is used to evaluate your retriever's quality on the amount of context retrieved. It uses LLM-as-a-judge to see if the retrieved context is enough to give an answer similar to expected output.
...
The contextual recall metric first extracts distinct statements from the expected output using an LLM. After that it uses the same LLM to see how many independent statements can be attributed to any of the nodes in the retrieved context.
