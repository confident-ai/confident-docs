...
The faithfulness metric is a single-turn RAG-based metric that is used to evaluate your generator's quality on it's ability to generate answers that are based only on the retrieved context. It uses LLM-as-a-judge to check if the actual output is only based on the retrieved context and does not hallucinate or provide misinformation.
...
The faithfulness metric first extracts individual statements as claims from the actual output using an LLM. It then uses the same LLM to check how many claims are supported by the retrieved context.
