...
The task completion metric uses LLM-as-a-judge to evaluate how effectively an LLM agent accomplishes a task. It is a single-turn metric to evaluate an agent's task completion capability.
...
The task completion metric first extracts task and outcome from the trace using an LLM. It then uses the same LLM to find the alignment score of task and outcome.
