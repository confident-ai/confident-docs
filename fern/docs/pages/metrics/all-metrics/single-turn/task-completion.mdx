...
The task completion metric is a single-turn metric that evaluates your LLM agent's entire trace to evaluate its ability on completeing the task given to it. It uses LLM-as-a-judge to see if your LLM agent has ultimately finished the task given to it.
...
The task completion metric uses an LLM to extract the task and outcome of each step in the trace. After which it uses the same LLM to see if the task extracted has been satisfied given the outcome of that step.
