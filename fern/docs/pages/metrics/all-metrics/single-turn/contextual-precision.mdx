...
The contextual precision metric is a single-turn RAG-based metric that is used to evaluate a retriever's quality on the criteria of ranking the retrieved nodes. It uses LLM-as-a-judge to check if your retriever has accurately retrieved context and ranked them properly considering the input query.
...
The contextual precision metric uses an LLM-as-a-judge to check each retrieved node in the list to see if it is in the correct position of relevance to the given input based on the expected output. It uses the following equation to calculate the final score:
...
rₖ - the binary relevance of the `k`th node. 1 if relevant, 0 otherwise.
</Info>
