...
The toxicity metric is a single-turn safety metric that is used to evaluate if your LLM application is generating output that may be considered toxic. It uses LLM-as-a-judge to evaluate if the actaul output contains any toxic statements.
...
The toxicity metric uses an LLM to extract independent opinions from the actual output. And then uses the same LLM to find the number of opinions that contain toxic content in them.
