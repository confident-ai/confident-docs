...
The answer relevancy metric is a single-turn RAG-based metric that is used to evaluate a generator's output in a RAG application for a given input. It uses LLM-as-a-judge to make evaluations to see if the answer given by your RAG agent is relevant to the question asked in the input.
...
The answer relevancy metric breaks down the actual output in your test case into individual statements using an LLM, after which it uses the same LLM to check if the statements are relevant to the input supplied.
