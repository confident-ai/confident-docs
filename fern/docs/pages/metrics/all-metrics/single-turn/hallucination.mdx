...
The hallucination metric is a single-turn safety metric that is used to check if your LLM application is generating outputs that are false or hallucinated. It uses LLM-as-a-judge to check if the actual output is truthful to the context provided.
...
The Hallucination metric uses an LLM to determine if there are any contradictions made to the contexts from the actual output. Here the context provided is used as the ground truth.
