...
The turn relevancy metric is a multi-turn metric that is used to evaluate whether your chatbot provides relevant outputs for a given user input in turns. It uses LLM-as-a-judge to evaluate if your all the chatbot turns in the conversation are relevant to the corresponding user's inputs.
