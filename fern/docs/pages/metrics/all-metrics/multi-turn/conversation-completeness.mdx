...
The conversational completeness is a multi-turn metric that is used to evaluate whether your chatbot is satisfying the user in each turn. It uses LLM-as-a-judge to see if the chatbot meets the user's requirements throughout the conversation.
...
The conversation completeness metric fist extracts distinct user intentions from all the turns using an LLM. It then uses the same LLM to see if the corresponding assistant's turn has satisfied the user's intentions.
