...
<Tabs items={['Manual Cost Tracking', 'Automatic Cost Tracking']}>
  <Tabs.Tab>
```python showLineNumbers {6,7,12,13} filename="main.py"
...
```
  ...
  <Tabs.Tab>
```python showLineNumbers {5, 10, 11} filename="main.py"
...
To set up automatic cost tracking, provide the `model` in the `@observe` decorator of your [LLM span](/docs/llm-tracing/tracing-features/span-types#llm-span), and provide the `input` and `output` in your `update_current_span`.
...
To manually set up cost tracking, provide the `cost_per_input_token` and `cost_per_output_token` in the `@observe` decorator of your [LLM span](/docs/llm-tracing/tracing-features/span-types#llm-span), and pass the input and output token counts via `update_llm_span`.
...
```python showLineNumbers {6,7,12,13} filename="main.py"
from deepeval.tracing import update_current_span, update_llm_span
