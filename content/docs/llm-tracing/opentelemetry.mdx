...
You can configure the enviornment and sampling rate of traces by setting the following two enviornment variables:

```bash
CONFIDENT_TRACE_ENVIRONMENT="staging"
CONFIDENT_SAMPLE_RATE=0.5
```

The [enviornment](/docs/llm-tracing/tracing-features/environment) allows you to configure where your trace belongs on Confident AI (defaulted to `"development"`), whereas the [sample rate](/docs/llm-tracing/tracing-features/sampling) is a number between 0-1 that specifies the probability of a certain trace being ingested to the platform (defaulted to 1). 
