...
You'll also need to use `update_current_span` with [test case parameters](/docs/llm-tracing/tracing-features/test-cases) at runtime to actually trigger an online evaluation on the server-side:
...
```
  ...
  <Tabs.Tab>
```typescript filename="index.ts" showLineNumbers copy {4,11}
import { observe, updateCurrentSpan } from '@deepeval-ts/tracing';

const llmApp = async (prompt: string) => {
...
You can also set the retrieval context and tools called (if any) for each assistant/AI turn, which would allow you to see the context of which an LLM generation was invoked in:
