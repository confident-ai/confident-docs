...
def llm(input: str) -> str:
    with tracer.start_as_current_span("llm_span") as span:
        ...
        span.set_attribute("confident.llm.span.input", [
            json.dumps({"role": "system", "content": "You are a helpful assistant."}),
            json.dumps({"role": "user", "content": input})
        ])
        time.sleep(0.5)
        span.set_attribute("confident.llm.span.output", "Hello world")
...
- `confident.trace.tags` (of type `list[str]`) used for updating trace [tags](/docs/llm-tracing/tracing-features/tags).
- `confident.trace.thread_id` (of type `str`) used for updating trace [thread id](/docs/llm-tracing/tracing-features/threads).
- `confident.trace.user_id` (of type `str`) used for updating trace [user id](/docs/llm-tracing/tracing-features/users).
