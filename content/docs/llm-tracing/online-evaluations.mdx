...
You can do this by providing the name of the **metric collection** you've created on Confident AI in the `@observe` decorators during tracing.
...
If you're planning to run online evals on threads, create a multi-turn metric collection, and vice versa. This is really important as you'll need the metric collection names to match as you'll see in the next section.

<Callout>
Only **referenceless** metrics you've enabled inside the metric collection are runnable upon tracing. For non-referenceless metrics, Confident AI will simply ignore them.
...
Simply provide the name of the metric collection to tell Confident AI the specific set of referenceless metrics you wish to run in the `@observe` decorator. 
...
The `metricCollection` argument is an optional string that determines which metrics in your online metric collection will be run for this current span. 
