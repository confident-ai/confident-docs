...
You can log a **single-turn LLM interaction** through LLM tracing. Traces are a single execution of your LLM app, and running evals on traces is akin to the [end-to-end evals](/docs/llm-evaluation/run-evals/end-to-end-evals) for LLM evaluation in development.
...
Spans are **individual components** such as LLM calls, agents, retrieveres, tools, etc. that make up a trace. Running evals on spans is akin to the [component-level evals](/docs/llm-evaluation/run-evals/component-level-evals) for LLM evaluation in development.
