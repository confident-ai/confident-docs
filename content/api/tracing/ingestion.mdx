...
<Callout>
Individual spans cannot be updated.
</Callout>

## Batch Ingestion

You can also send batch ingest traces to Confident AI, accepting the the same [`Trace` data model](/api/tracing/data-models) via the `"traces"` key:

```bash filename="POST - /v1/batch-traces" showLineNumbers {5}
curl -X POST "https://api.confident-ai.com/v1/batch-traces" \
     -H "Content-Type: application/json" \
     -H "CONFIDENT_API_KEY: <PROJECT-API-KEY>" \
     -d '{
            "traces": [{
...
[Online evaluations](/api/tracing/evaluations) work exactly the same way as before via the batch ingestion API.