...
```python showLineNumbers filename="main.py" {12, 13, 14, 23}
import os
import asyncio
import time
...
from deepeval.integrations import LlamaIndexSpanHandler, LlamaIndexEventHandler
import deepeval
...
<Callout>
As the agent is running, it will export **spans** and **events** to the `dispatcher`. The `dispatcher` will send the spans to the `LlamaIndexSpanHandler` and the events to the `LlamaIndexEventHandler`, which will collect them to create a Confident AI **trace** and send it to the [Observatory](https://documentation.confident-ai.com/llm-tracing/introduction).
</Callout>
